...
# S&P Global publishes Caixin PMI in English
_SP_GLOBAL_URL = "https://www.pmi.spglobal.com/Public/Home/PressRelease"
...
class CaixinFetcher(BaseFetcher):
    ...
    async def fetch_latest(self) -> list[FetchResult]:
        html = await self._get_html(_SP_GLOBAL_URL)
        soup = BeautifulSoup(html, "html.parser")

        for a in soup.find_all("a", href=True):
            text = a.get_text(strip=True).lower()
            if "caixin" in text and ("pmi" in text or "china" in text):
                url = a["href"]
                if not url.startswith("http"):
                    url = "https://www.pmi.spglobal.com" + url
                try:
